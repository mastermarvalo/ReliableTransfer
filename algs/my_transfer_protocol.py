...
class MyTransferProtocol:
    ...
    def run_server(self, outdir, addr, mtu):
        ...
        #increase MTU to support checksum
        mtu = mtu + 8
        ...
        while True:
            ...
            if in_xfr:
                ...
                if data[:9] == B"///END\\\\\\":
                    ...
                    #debug info for me to keep track of 
                    log.debug("Sent Data: " + str(data[4:]))
                    log.debug("Checksum: " + str(data[:4]))

                    #create response in format checksum + ACK
                    response = data[:4] + B'ACK'

                    #check if checksum is valid
                    if data[:4] == binascii.crc32(data[4:]).to_bytes(4, byteorder='little'):
                        # just write the data...
                    ...
                    # and send a the response
                    sock.sendto(response, remote_addr)
            else:
    ...
    def xfr(self, sock, payload, dest, mtu):
        ...
        for i,chunk in enumerate(payload):
            ...
            while tries < self.retries:
                ...
                #create a checksum to verify packet order and prevent duplicates
                checksum = binascii.crc32(chunk)
                log.debug("Checksum: " + str(checksum.to_bytes(4, byteorder='little')))
                #apply that checksum to the initial chunk in proper byte order
                initial_chunk = checksum.to_bytes(4, byteorder='little') + chunk

                #response to expect back of checksum + ACK
                expected_response = checksum.to_bytes(4, byteorder='little') + B'ACK'

                # send the chunk
                sock.sendto(initial_chunk, dest)
                try:
                    # wait for an ACK
                    data, addr = sock.recvfrom(mtu)
                except socket.timeout:
                ...
                if data == expected_response:
                    ...
                    log.info("Bad response from server, got {} instead of {}, RETRY".format(
                        data, checksum.to_bytes(4, byteorder='little') + B"ACK"))
            if (tries >= self.retries):
    ...
    def send_file(self, filename, dest, mtu):
        ...
        st = datetime.now()
        log.info("Sending with my-protocol {} --> {}:{} [MTU={}].".format(
